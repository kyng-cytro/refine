...
const existing = await db
  .select({ slug: schema.tones.slug })
  .from(schema.tones)
  .where(eq(schema.tones.isGlobal, true))
const have = new Set(existing.map((t) => t.slug))
const missing = GLOBAL_TONES.filter((t) => !have.has(t.slug))
if (missing.length) await db.insert(schema.tones).values(missing)

console.log(`Seeding complete. Added ${missing.length} tone(s).`)
client.close()
