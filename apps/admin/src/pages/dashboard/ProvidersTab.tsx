...
export default function ProvidersTab() {
  const [forms, setForms] =
    useState<Record<ModelProvider, FormState>>(defaultForms)
  ...
  useEffect(() => {
    api.providers
      .list()
      .then((providerStates: ProviderState[]) => {
        const modelMap: Record<string, boolean> = {}
        const stateMap: Partial<
          Record<ModelProvider, { enabled: boolean; hasKey: boolean }>
        > = {}
        providerStates.forEach((p) => {
          stateMap[p.provider as ModelProvider] = {
            enabled: p.enabled,
            hasKey: p.hasKey,
          }
          p.models.forEach((m) => {
            modelMap[m.id] = m.enabled
          })
        })
        setForms(
          (prev) =>
            Object.fromEntries(
              PROVIDERS.map((p) => [
                ...
                {
                  ...prev[p.id],
                  enabled: stateMap[p.id]?.enabled ?? false,
                  hasKey: stateMap[p.id]?.hasKey ?? false,
                },
              ]),
            ) as Record<ModelProvider, FormState>,
        )
  ...
  const save = async (p: ModelProvider) => {
    ...
    setForms((prev) => ({
      ...prev,
      [p]: { ...prev[p], saving: true, saved: false },
    }))
    setError("")
    try {
      ...
      setTimeout(
        () =>
          setForms((prev) => ({ ...prev, [p]: { ...prev[p], saved: false } })),
        2000,
      )
    } catch (err) {
  ...
  const entries: ProviderEntry[] = PROVIDERS.map((p) => ({
    ...
    onKeyChange: (v) =>
      setForms((prev) => ({ ...prev, [p.id]: { ...prev[p.id], apiKey: v } })),
    onToggleShow: () =>
      setForms((prev) => ({
        ...prev,
        [p.id]: { ...prev[p.id], showKey: !prev[p.id].showKey },
      })),
    onSave: () => save(p.id),
