...
export default function StepProviders({ onNext }: Props) {
  ...
  const [showKey, setShowKey] = useState<Record<ModelProvider, boolean>>({
    openai: false,
    anthropic: false,
    google: false,
  })
  ...
  const toggleModel = (id: string, provider: ModelProvider) => {
    if (!hasKey(provider)) return
    setModels((prev) => ({ ...prev, [id]: !prev[id] }))
  }
                ...
                >
                  <AccordionTrigger className="hover:no-underline py-4 [&>svg]:hidden">
                    <div className="flex items-center gap-4">
                      <div className="flex h-11 w-11 items-center justify-center rounded-lg bg-muted text-foreground shrink-0">
                        {meta.logo}
                      </div>
                  ...
                  <AccordionContent className="pb-4 space-y-4">
                    ...
                    <div className="space-y-1">
                      ...
                      {MODELS.filter((m) => m.provider === p).map((m) => {
                        const active = hasKey(p) && models[m.id]
                        return (
                          <div
                            key={m.id}
